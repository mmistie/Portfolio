...
class DetailViewController: UIViewController, CoordinatedViewController {
    ...
    @IBAction func yourButtonTapped(_ sender: Any) {
            // Perform the segue when the button is tapped
            performSegue(withIdentifier: "ToMoreDetails", sender: self)
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ToMoreDetails" {
            if let thirdViewController = segue.destination as? ThirdViewController {
                // Pass the data you want to the thirdViewController
                thirdViewController.titleFromDetailVC = titleLabel.text
                thirdViewController.yearFromDetailVC = yearLabel.text
                thirdViewController.plotFromDetailVC = movieDetail?.plot
            }
        }
    }
}
